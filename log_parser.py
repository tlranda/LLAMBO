...
def logparse(logfile):
    ...
    print(icl)
    print("And here they are sorted for ease of comparison:")
    print(sorted(icl))
    ...
    average_difference = np.abs(llm_responses.mean(axis=1) - ground_truth)
    print(f"Average difference of (MEAN LLM response per query vs query's ground truth) per Ground Truth")
    print(average_difference.mean())
    print("Sorted Ground Truths:")
    print(sorted(ground_truth))
    print(f"Difference per Ground Truth in case it's only good at predicting certain zones:")
    print(average_difference[np.argsort(ground_truth)])
    ...
    print(dict((k, llm_flat.count(k)) for k in sorted(unique_llm_generations)))

...
def get_bayesmark_func(model_name, task_type, y_train=None):
    ...
    assert model_name in ['RandomForest', 'DecisionTree', 'SVM', 'AdaBoost', 'MLP_SGD',
                          'DatasetIdentity:syr2k', 'DatasetIdentity:syr2k_q', 'DatasetIdentity:syr2k_r'],\
            f'Unknown model name: {model_name}'
    assert task_type in ['classification', 'regression', 'quantile-prediction', 'rank-prediction']
    if model_name == 'RandomForest' or model_name.startswith('DatasetIdentity:syr2k'):
